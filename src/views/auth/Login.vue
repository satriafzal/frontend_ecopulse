<template>
  <div class="min-h-screen bg-dark-purple flex items-center justify-center relative overflow-hidden px-4 pt-28"
    ...
    <div
      class="absolute top-[-10%] left-[-10%] w-96 h-96 bg-neon-purple rounded-full mix-blend-screen filter blur-[120px] opacity-30 animate-pulse">
    </div>
    <div
      class="absolute bottom-[-10%] right-[-10%] w-96 h-96 bg-neon-cyan rounded-full mix-blend-screen filter blur-[120px] opacity-30">
    </div>

    <div
      class="w-full max-w-md bg-[#0a012a]/60 backdrop-blur-xl border border-dark-border rounded-3xl p-10 z-10 shadow-2xl shadow-neon-purple/10">
      ...
      <div class="mt-8 text-center text-sm text-gray-400">
        Belum punya akun?
        <RouterLink to="/register" class="text-neon-magenta hover:underline font-semibold ml-1">Daftar Sekarang
        </RouterLink>
      </div>
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (response.data.role === 'admin') {
        toast.add({
          severity: 'success',
          summary: 'Login Admin Berhasil!',
          detail: 'Selamat datang di Ruang Kendali EcoPulse',
          life: 4000
        });
      } else {
        toast.add({
      ...
      setTimeout(() => {
        if (response.data.role === 'admin') {
          router.push('/admin/adm-dashboard');
        } else {
